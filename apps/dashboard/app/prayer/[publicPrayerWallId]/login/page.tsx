...
import {
  ...
  brandInputClasses,
  brandInputStyle,
  DEFAULT_PRAYER_WALL_BRAND_COLOR,
} from "../../../../lib/prayer-branding";
...
async function loginAction(publicPrayerWallId: string, formData: FormData) {
  "use server";
  const wall =
    await prayerWallService.resolvePublicPrayerWall(publicPrayerWallId);
  ...
  const rateCheck = checkRateLimit(
    `prayer-login:${wall.organizationId}:${ip ?? "unknown"}`,
    10,
    60 * 60 * 1000,
  );
  if (!rateCheck.allowed)
    redirect(
      `/prayer/${publicPrayerWallId}/login?error=rate_limited${nextQuery}`,
    );
  ...
  const account = await prayerService.findAccountByEmail(
    wall.organizationId,
    email,
  );
  ...
  const token = await prayerService.createSession(
    account.id,
...
export default async function PrayerLoginPage({
  ...
  const brandColor =
    wall.prayerWallBrandColor ?? DEFAULT_PRAYER_WALL_BRAND_COLOR;
  const hasCustomBrandColor = brandColor !== DEFAULT_PRAYER_WALL_BRAND_COLOR;

  return (
    <div
      className={`min-h-screen ${hasCustomBrandColor ? "bg-surface" : "bg-surface-muted"}`}
    >
      <PrayerWallHeader
        ...
        testimoniesPageName={
          wall.testimoniesEnabled ? wall.testimoniesPageName : undefined
        }
        isLoggedIn={false}
      />
      <main className="mx-auto max-w-sm px-6 py-12">
        <PrayerPageIntro
          ...
          brandColor={brandColor}
        />
        {sp.next === "submit" && !sp.error && (
          <p className="-mt-2 mb-4 text-sm text-ink-secondary">
            A quick free account keeps spam off the wall -- it only takes a
            minute, and your name is never shown publicly unless you choose to
            share it.
          </p>
        )}
        {sp.error && (
        ...
        <p className="mt-4 flex flex-col gap-1.5 text-sm text-ink-secondary">
          <a
            href={`/prayer/${publicPrayerWallId}/forgot-password`}
