...
import { formService, groupService, parseSchema, parseWorkflowConfig, personDisplayName, workflowService } from "@ruach/database";
...
export default async function WorkflowDetailPage({ params }: { params: Promise<{ workflowId: string }> }) {
  ...
  const [groups, forms, runs] = await Promise.all([
