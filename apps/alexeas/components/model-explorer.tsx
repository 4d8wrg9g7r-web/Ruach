 ...
 * Each model is shown as a richly-rendered instrument (its recommended woods),
 * set on a warm plinth of light; it lifts gently on hover. Personality leads,
 * specifications stay quiet.
 */
export function ModelExplorer() {
  return (
    <div className="divide-y divide-brass/20">
      {MODELS.map((model, i) => (
        <article key={model.slug} className="reveal grid items-center gap-8 py-16 md:grid-cols-[0.9fr_1.4fr] md:gap-16">
          <div className={`group flex justify-center ${i % 2 === 1 ? "md:order-2" : ""}`}>
            <Link
              href={`/instruments/${model.slug}`}
              aria-label={`View the ${model.name}`}
              className="relative block"
            >
              <span
                aria-hidden
                className="absolute inset-0 -z-10 scale-90 rounded-full opacity-0 blur-2xl transition-opacity duration-1000 group-hover:opacity-100"
                style={{ background: "radial-gradient(circle, rgba(168,138,90,0.28), transparent 70%)" }}
              />
              <InstrumentPreview
                config={defaultConfig(model.slug)}
                uid={`me-${model.slug}`}
                className="h-64 w-auto drop-shadow-[0_28px_40px_rgba(36,28,23,0.22)] transition-transform duration-1000 ease-atelier group-hover:-translate-y-2 group-hover:rotate-[2deg] sm:h-72"
              />
            </Link>
          </div>
          ...
          <div className={i % 2 === 1 ? "md:order-1" : ""}>
            <div className="flex items-baseline gap-4">
              <span className="mono-num text-sm text-brass">0{model.order}</span>
              <h3 className="font-serif text-5xl text-espresso sm:text-6xl">{model.name}</h3>
            </div>
