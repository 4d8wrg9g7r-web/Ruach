...
const brandColorSchema = z
  .string()
  .regex(/^#[0-9a-fA-F]{6}$/, "must be a 6-digit hex color like #b87b38");
...
// Same rule as accountEmailSchema (this is the org's own contact address, not the
// signed-in user's) -- reused rather than a second copy that could drift.
const contactEmailSchema = accountEmailSchema;
...
async function updateAccountAction(formData: FormData) {
  ...
  const nameParsed = accountNameSchema.safeParse(
    String(formData.get("name") ?? ""),
  );
  if (!nameParsed.success)
    throw new Error(nameParsed.error.issues[0]?.message ?? "Invalid name.");

  const emailParsed = accountEmailSchema.safeParse(
  ...
  await unstable_update({
    user: { name: nameParsed.data, email: emailParsed.data },
  });
...
async function updatePublicContactAction(formData: FormData) {
  "use server";
  const organization = await getCurrentOrganization();
  ...
  if (rawEmail) {
    const parsed = contactEmailSchema.safeParse(rawEmail);
    if (!parsed.success)
      throw new Error(parsed.error.issues[0]?.message ?? "Invalid email.");
    contactEmail = parsed.data;
  }

  const rawUrl = String(formData.get("publicWebsiteUrl") ?? "").trim();
  let publicWebsiteUrl: string | null = null;
  if (rawUrl) {
    const parsed = publicWebsiteUrlSchema.safeParse(rawUrl);
    if (!parsed.success)
      throw new Error(parsed.error.issues[0]?.message ?? "Invalid URL.");
    publicWebsiteUrl = parsed.data;
  }

  await organizationService.setPublicContactInfo(organization.id, {
    contactEmail,
    publicWebsiteUrl,
  });
  ...
  await auditService.recordAuditEvent({
    organizationId: organization.id,
    actorUserId: user?.id,
    action: "organization.contact_info_updated",
...
async function enablePrayerWallAction(formData: FormData) {
  ...
  const maxForwardingEmails = billingService.planHasFeature(
  ...
  if (
    ...
    forwardingEmails.length > maxForwardingEmails
  ) {
    throw new Error(
      `Your plan supports up to ${maxForwardingEmails} forwarding address. Upgrade to notify more people.`,
    );
  }
  ...
  if (rawBrandColor) {
    ...
    if (!parsed.success)
      throw new Error("Brand color must be a 6-digit hex color, like #b87b38.");
  ...
  const testimoniesPageName =
    String(formData.get("testimoniesPageName") ?? "").trim() || "Praise Report";
    ...
    <div>
      ...
      <Card padding="md" className="mb-6">
        ...
        <p className="mb-4 text-sm text-ink-secondary">
          Shown to a visitor when the assistant can&rsquo;t answer their
          question from your indexed content, and to politely redirect a
          business pitch or sales message that isn&rsquo;t a real question about
          your church. Never shown as a resource recommendation or search
          result.
        </p>
        <PublicContactForm
      ...
      <Card padding="md" className="mb-6">
        ...
        <p className="mb-4 text-sm text-ink-secondary">
          A public page where visitors can submit prayer requests and, if they
          choose, post them publicly for others to pray for. New submissions can
          be forwarded to a staff email.
          {billingService.planHasFeature(
            organization.planKey,
            "campusScopedPrayerWalls",
          ) && (
            <>
              {" "}
              This is the org-wide default wall -- give an individual campus its
              own wall on the{" "}
              <a href="/websites" className="underline hover:text-ink">
                Websites
              </a>{" "}
              page.
            </>
        ...
        <PrayerWallSettingsForm
          ...
          brandColor={
            organization.prayerWallBrandColor ?? DEFAULT_PRAYER_WALL_BRAND_COLOR
          }
      ...
      <p className="mt-6 text-xs text-ink-muted">
        Manage teammates on the{" "}
        <a href="/team" className="underline hover:text-ink-secondary">
          Team
        </a>{" "}
        page, usage on the{" "}
        <a href="/billing" className="underline hover:text-ink-secondary">
          Billing
        </a>{" "}
        page, and activity history on the{" "}
        <a href="/audit-log" className="underline hover:text-ink-secondary">
          Audit log
        </a>{" "}
        page. Safety configuration is still on the roadmap -- see
        docs/admin-guide.md.
      </p>
    </div>
  );
