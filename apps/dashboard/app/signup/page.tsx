...
async function signupAction(formData: FormData) {
  ...
  const accessCode = code
    ? await accessCodeService.getValidAccessCode(code)
    : null;
  if (code && !accessCode)
    redirect(`/signup?error=invalid_code&code=${encodeURIComponent(code)}`);
  ...
  if (accessCode) {
    ...
    const organization =
      await organizationService.createOrganizationFromAccessCode({
        name: orgName,
        slug,
        ownerUserId: user.id,
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-sm flex-col justify-center px-4 py-12">
      ...
      <p className="mb-6 text-sm text-ink-secondary">
        Set up your organization&rsquo;s workspace on Ruach.
      </p>
      {sp.error && (
      ...
      <Card>
        <ValidatedForm action={signupAction} className="flex flex-col gap-3">
          ...
          <label className="text-sm font-medium text-ink-secondary">
            ...
            <span className="font-normal text-ink-muted">
              (optional -- skip if you don&rsquo;t have one)
            </span>
            <Input
              name="code"
